...
import { delay, Observable, Subject, Subscription, takeUntil, tap } from 'rxjs';
...
import { EventDetailSidebarComponent } from './components/event-detail-sidebar/event-detail-sidebar.component';
...
import { EventCardViewComponent } from './components/event-card-view/event-card-view.component';
...
const components = [
  WellcomeConfettiComponent, EventCardViewComponent, MasterTableComponent
]

@Component({
  selector: 'app-event-list',
  imports: [FormsModule, ColDefDirective, DatePipe, SharedModule, ...antDesignModules, ...components],
  providers: [],
...
export class EventListComponent implements AfterViewInit, OnInit, OnDestroy {
          ...
          this.querySubscription = this._activeRoute.queryParams.subscribe((params) => {
            if (params['eventId']) {
              ...
              if (foundEvent) {
                ...
                this._message.error('Event not found');
              }
            }
          })
        }
  ...
  getEventsTableData(): Observable<any> {
